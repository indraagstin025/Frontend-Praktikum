...
export function Navbar({ onMenuClick, sidebarOpen, isDesktop }) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 h-16 bg-white shadow-md flex items-center transition-all duration-300">
      ...
      {/* Container Welcome */}
      <div className={`flex items-center h-full transition-all duration-300 ${isDesktop && sidebarOpen ? "ml-64 pl-6" : "ml-4"}`}>
      ...
      <div className="mr-2 flex items-center justify-end flex-1">
        <button onClick={handleLogout} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md text-sm transition">
          Logout
        </button>
      </div>
    </header>
